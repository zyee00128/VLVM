...
class BaseObjectNavPolicy(BasePolicy):
    ...
    def _update_object_map(
        ...
        # When the physical depth sensor returns abnormally empty depth (an all-ones
        # matrix) but the 2D detector confirms a target exists, trigger the depth
        # estimator to recover the depth and update the cache.
        ...
        # Double filtering via SAM pixel-level segmentation and VQA verification
        ...
        # Compute the camera field of view (FOV) and mark the space inside the
        # viewing cone as explored on the global point-cloud map
...
class VLFMConfig:
    ...
    text_prompt: str = "Seems like there is a target_object ahead."  # Language prompt used for VQA-based target confirmation.
    pointnav_policy_path: str = "data/pointnav_weights.pth"          # Path to the pretrained PointNav weights.
    depth_image_shape: Tuple[int, int] = (224, 224)                   # (H, W) resolution fed to the PointNav policy; larger = more detail but slower.
    pointnav_stop_radius: float = 0.9         # Stop distance (m) to the goal; larger = stops farther from the target.
    use_max_confidence: bool = False          # Use global max confidence for value-map projection; True keeps only the peak response.
    object_map_erosion_size: int = 5          # Erosion kernel size (px) applied to object masks; larger = shrinks masks more (removes boundary noise).
    exploration_thresh: float = 0.0           # Similarity threshold to switch into target-chasing; higher = more conservative chasing.
    obstacle_map_area_threshold: float = 1.5  # Frontier area threshold (m^2); higher = fewer, larger frontiers.
    min_obstacle_height: float = 0.61         # Lower height bound (m) for obstacle points; too low includes floor noise.
    max_obstacle_height: float = 0.88         # Upper height bound (m) for obstacle points; too low ignores tall obstacles.
    hole_area_thresh: int = 100000            # Hole area threshold (px) for depth gap filling; -1 disables filling.
    use_vqa: bool = False                     # Enable BLIP2 VQA verification of segmented objects; True improves precision but adds latency.
    vqa_prompt: str = "Is this "              # Question prefix used for VQA verification.
    coco_threshold: float = 0.8               # Confidence threshold for COCO detections; higher = fewer, more reliable detections.
    non_coco_threshold: float = 0.4           # Confidence threshold for open-vocabulary detections; higher = stricter, fewer detections.
