...
from habitat_baselines.utils.info_dict import extract_scalars_from_info as extract_scalars_from_info_habitat
...
class VLVMTrainer(PPOTrainer):
    ...
    def _eval_checkpoint(
        ...
        # Initialize per-environment timers and counters
        env_step_counts = [0] * self.envs.num_envs
        env_policy_times = [0.0] * self.envs.num_envs
        env_total_times = [0.0] * self.envs.num_envs

        while len(stats_episodes) < (number_of_eval_episodes * evals_per_ep) and self.envs.num_envs > 0:
            current_episodes_info = self.envs.current_episodes()

            # Measure the forward-inference time of the 3D navigation policy
            ...
            with inference_mode():
                ...
                if "VLVM_RECORD_ACTIONS_DIR" in os.environ:
                    action_id = action_data.actions.cpu()[0].item()
                    filepath = os.path.join(
                        os.environ["VLVM_RECORD_ACTIONS_DIR"],
                        "actions.txt",
                    )
            ...
            # Measure the single-step execution time of the physics simulation engine
            t2 = time.perf_counter()
            outputs = self.envs.step(step_data)
            t3 = time.perf_counter()
            env_latency = t3 - t2
            total_step_latency = policy_latency + env_latency
            ...
            for i in range(n_envs):
                env_step_counts[i] += 1
                env_policy_times[i] += policy_latency
                env_total_times[i] += total_step_latency
            ...
            for i in range(n_envs):
                ...
                if not not_done_masks[i].item():
                    ...
                    episode_stats["steps_count"] = env_step_counts[i]
                    episode_stats["avg_step_latency_s"] = env_total_times[i] / max(1, env_step_counts[i])
                    episode_stats["avg_policy_latency_s"] = env_policy_times[i] / max(1, env_step_counts[i])
                    episode_stats["fps"] = env_step_counts[i] / max(1e-4, env_total_times[i])
                    
                    if torch.cuda.is_available():
                        episode_stats["peak_gpu_mem_mb"] = torch.cuda.max_memory_allocated() / (1024 ** 2)
                    else:
                        episode_stats["peak_gpu_mem_mb"] = 0.0

                    if psutil is not None:
                        process = psutil.Process(os.getpid())
                        episode_stats["peak_cpu_mem_mb"] = process.memory_info().rss / (1024 ** 2)
                    else:
                        episode_stats["peak_cpu_mem_mb"] = 0.0

                    env_step_counts[i] = 0
                    env_policy_times[i] = 0.0
                    env_total_times[i] = 0.0
                    ...
                    try:
                        ...
                        traceback.print_exc()
                        print(
                            f"[EpisodeStatsLogger] log_episode_stats raised "
                            f"{type(e).__name__}: {e}",
                            flush=True,
                        )
            ...
            # When pausing and removing environments (Pause Vectorized Env), keep
            # the per-env timing containers in sync
            if len(envs_to_pause) > 0:
                env_step_counts = [val for idx, val in enumerate(env_step_counts) if idx not in envs_to_pause]
                env_policy_times = [val for idx, val in enumerate(env_policy_times) if idx not in envs_to_pause]
                env_total_times = [val for idx, val in enumerate(env_total_times) if idx not in envs_to_pause]
