...
class HabitatVis:
    ...
    def _reorient_rescale_habitat_map(infos: List[Dict[str, Any]], vis_map: np.ndarray) -> np.ndarray:
        ...
        start_yaw = infos[0].get("start_yaw", 0.0)
        if start_yaw != 0.0:
...
def color_point_cloud_on_map(infos: List[Dict[str, Any]], policy_info: List[Dict[str, Any]]) -> None:
    ...
    #     return
    if (
        "target_point_cloud" not in policy_info[0]
        or policy_info[0]["target_point_cloud"] is None
        or len(policy_info[0]["target_point_cloud"]) == 0
    ):
        return
    ...
    cloud_episodic_frame = np.array(policy_info[0]["target_point_cloud"])[:, :3]
