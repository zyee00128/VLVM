...
class BLIP2ITM:
    ...
    def __init__(
        self,
        name: str = "blip2_image_text_matching",  # BLIP-2 model family name
        model_type: str = "pretrain",             # Pretrained checkpoint variant; larger = stronger but slower
    ...
    def cosine(self, image: np.ndarray, txt: str) -> float:
        ...
        img = self.vis_processors["eval"](pil_img).unsqueeze(0).to(self.device)  # Convert the image to a PyTorch tensor and add a batch axis.
        txt = self.text_processors["eval"](txt)  # Normalize the text and convert it to PyTorch format.
...
class BLIP2ITMClient:
    ...
    def cosine(self, image: np.ndarray, txt: str) -> float:
        ...
        print(f"BLIP2ITMClient.cosine({txt!r}): {score:.4f}", flush=True)
        return score
