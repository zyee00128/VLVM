...
class BLIP2:
    def __init__(
        self,
        name: str = "blip2_t5",          # BLIP-2 model family name
        model_type: str = "pretrain_flant5xxl",  # Pretrained checkpoint variant; larger = stronger but slower
        device: Optional[Any] = None,    # Compute device; defaults to CUDA if available
    ) -> None:
        if device is None:
    ...
    def ask(self, image: np.ndarray, prompt: Optional[str] = None) -> str:
        ...
        """
        pil_img = Image.fromarray(image)  # Convert the NumPy RGB image array to a PIL image.
        with torch.inference_mode():
            processed_image = self.vis_processors["eval"](pil_img).unsqueeze(0).to(self.device)  # Apply the model's eval preprocessor (resize, normalize, to tensor) and add a batch axis.
            # Without a prompt, generate a global image caption; with a prompt, pass it in for image-guided answering.
