...
class BLIP2Client:
    ...
    def is_available(self, timeout: float = 1.0) -> bool:
        """Lightweight TCP probe: True if the BLIP2 VQA server is listening.

        Avoids the 10x/20-30s retry + exit() inside send_request when the
        server was never started."""
        try:
            with socket.create_connection(("localhost", self._port), timeout=timeout):
                return True
        except OSError:
            return False
